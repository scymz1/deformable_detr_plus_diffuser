...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        self.last_object_queries_transform = nn.Linear(d_model, clip_dim)
        ...
        if with_box_refine:
            ...
            # print(self.class_embed) # 6 x Linear(in_features=256, out_features=91, bias=True)
    ...
    def forward(self, samples: NestedTensor):
        ...
        # hs.shape 6,2,300,256 init_inference 6,1,300,256 ([300,256]->[300, 4], [300, 80])
        # [256] -> [x, y, w, h], [80]
        # print(hs.shape, init_reference.shape, inter_references.shape) # torch.Size([6, 1, 300, 256]) torch.Size([1, 300, 2]) torch.Size([6, 1, 300, 2])
        outputs_classes = []
        outputs_coords = []
        # print("hidden states")
        # print(hs.shape)
        for lvl in range(hs.shape[0]): # hs.shape = [6, 2, 300, 256]
            if lvl == 0:
                reference = init_reference
            else:
                reference = inter_references[lvl - 1]
            # print(reference.shape) # 1, 300 2
            reference = inverse_sigmoid(reference)
            outputs_class = self.class_embed[lvl](hs[lvl]) # outputs_class 2, 300, 91
        ...
        outputs_class = torch.stack(outputs_classes) # outputs_class 6, 2, 300, 91
        outputs_coord = torch.stack(outputs_coords)
        #outputs_class.shape [2, 300, 91]
        output_object_queries = self.last_object_queries_transform(hs)
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'object_query': output_object_queries[-1]}
        if self.aux_loss:
...
class SetCriterion(nn.Module):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'object_queries': self.loss_object_queries

        }
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)

    def forward(self, outputs, targets, save_recon_tensor = False, epoch = None):
        ...
        for loss in self.losses:
            kwargs = {"save_recon_tensor": save_recon_tensor, "epoch": epoch}
            # save_recon_tensor, epoch, 
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    if loss == 'masks' or loss == 'object_queries':
...
def build(args):
    ...
    weight_dict['loss_object_queries'] = args.object_queries_loss_coef
    if args.masks:
    ...
    losses = ['labels', 'boxes', 'cardinality'] # 'object_queries'
    if args.masks:
        losses += ["masks"]
...
def resize_img(image, max_dimension=60):
    ...
    if width > height:
        ...
        new_height = max(int(max_dimension / aspect_ratio), 1)
    else:
        new_height = max_dimension
        new_width = max(int(max_dimension * aspect_ratio), 1)
