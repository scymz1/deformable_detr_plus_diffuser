...
def load_image(imgname: str, target_size: Optional[Union[int, Tuple[int, int]]] = None) -> torch.Tensor:
    ...
    print(pil_img.size)
    if target_size == None:
        target_size = int(pil_img.size[-1] / 8) * 8
...
def ddim_inversion(imgname: str, num_steps: int = 50, verify: Optional[bool] = False) -> torch.Tensor:
    ...
    inverse_scheduler = DDIMInverseScheduler.from_pretrained("./diffuser_reconstruct/sd-coco-model", subfolder='scheduler')
    pipe = StableDiffusionPipeline.from_pretrained("./diffuser_reconstruct/sd-coco-model",
    ...
    # print(input_img.shape, latents.shape) # torch.Size([1, 3, 425, 640]) torch.Size([1, 4, 53, 80])
    ...
    # verify
    if verify:
        pipe.scheduler = DDIMScheduler.from_pretrained("./diffuser_reconstruct/sd-coco-model", subfolder='scheduler')
        image = pipe(prompt="an image of zebra", negative_prompt="", guidance_scale=1.,
                     num_inference_steps=num_steps, latents=inv_latents)
        # fig, ax = plt.subplots(1, 2)
        # ax[0].imshow(tvt.ToPILImage()(input_img[0]))
        # ax[1].imshow(image.images[0])
        print(image.images[0].save("reconstruct.jpg"))
        plt.show()
    return inv_latents
